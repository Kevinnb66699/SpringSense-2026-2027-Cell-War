...
- **蒙特卡洛 AI（实验）**：任一阵营可由"模拟对局打分"的推演搜索操控（原理与调参见 `docs/AI设计.md`），可对战或观战。
- **本地热座**：所有玩家共用一台电脑，按行动顺序轮流操作，与实体桌游体验一致。
- **观战演示**：双方均由 AI 操作，快速了解游戏流程。
- 窗口可任意拉伸/最大化，界面等比缩放。对局结束弹出结算页（胜方与关键数据），确认后返回主菜单。对局中按 **Esc** 打开暂停菜单（暂停 AI 行动、切换全屏、放弃对局返回主菜单、退出）。
...
- `game/scripts/core/` 为纯规则逻辑（不依赖场景节点，便于测试与将来联机复用）；`game/scripts/ui/` 为界面层逻辑。
- 界面静态布局与主题在 `game/scenes/`（`Main.tscn` / `SidePanel.tscn` / `MenuScreen.tscn` / `ui_theme.tres`），可在 Godot 编辑器里可视化调整，无需改代码。
